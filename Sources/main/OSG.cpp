...
-x Skip missing taxonomic header names
...
int main(int argc, char* argv[], char* envp[])
{
    ...
    std::vector<CFastaFile*> vecFastaFileHOGConcatenated; //Vector only containing fasta files that are concatenated HOGS into OGS. Will be filled in step 3.5 if applyable
    ...
    CheckReturnValue(lReturn, &vecFastaFilesSource, &vecFastaFileOrthoGroups, &vecFastaFileOrthoGroupsFiltered, &vecFastaFileHOGConcatenated, &vecOrthogroups, &vecTaxonomicGroups);
	CHECK_RETURNVALUE(lReturn);
    ...
    //STEP THREE AND HALF
    lReturn = glb.propertyBag.GetBoolOGInsteadofHOG(&bOGInsteadOfHOG);
    CheckReturnValue(lReturn, &vecFastaFilesSource, &vecFastaFileOrthoGroups, &vecFastaFileOrthoGroupsFiltered, &vecFastaFileHOGConcatenated, &vecOrthogroups, &vecTaxonomicGroups);
    CHECK_RETURNVALUE(lReturn);

    if (bOGInsteadOfHOG == true)
    {
        lReturn = ConcatenateHOGSIntoOGS(&vecFastaFileOrthoGroups, &vecFastaFileHOGConcatenated, &vecOrthogroups);
		CheckReturnValue(lReturn, &vecFastaFilesSource, &vecFastaFileOrthoGroups, &vecFastaFileOrthoGroupsFiltered, &vecFastaFileHOGConcatenated, &vecOrthogroups, &vecTaxonomicGroups);
        CHECK_RETURNVALUE(lReturn);
        sizeTotalOrthoGroups = vecFastaFileHOGConcatenated.size();
    }
    ...
    if (bOGInsteadOfHOG == false)
    {
        lReturn = FilterOrthoGroupsOutput(&vecFastaFileOrthoGroups, &vecFastaFileOrthoGroupsFiltered, pTaxonomicGroupSorted);
    }
    else
		lReturn = FilterOrthoGroupsOutput(&vecFastaFileHOGConcatenated, &vecFastaFileOrthoGroupsFiltered, pTaxonomicGroupSorted);
    CheckReturnValue(lReturn, &vecFastaFilesSource, &vecFastaFileOrthoGroups, &vecFastaFileOrthoGroupsFiltered, &vecFastaFileHOGConcatenated, &vecOrthogroups, &vecTaxonomicGroups);
	CHECK_RETURNVALUE(lReturn);
