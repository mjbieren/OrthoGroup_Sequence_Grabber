	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (*argv[i] == '-') //switch detected
		{
			...
			case 's': //SummaryFileOutput
				...
				aiRequirements[6] = 1;
				break;
			case 'x': //Skip non containing taxonomic headers instead of stopping the program.
				lRet = glb.propertyBag.SetBoolSkipTaxonomicGroup();
				if (lRet != ERR_NOERROR)
					return PARSECOMMANDLINE_BOOLSKIPTAXONOMICHEADER_OSG;
				aiRequirements[7] = 1;
				break;
			case 'c': //Let you combine all the HOGS into OGS
				lRet = glb.propertyBag.SetBoolOGInsteadofHOG();
				if (lRet != ERR_NOERROR)
					return 1;
				aiRequirements[8] = 1;
				break;
...
*/
		...
		else if (*argv[i] == '?')
		{
			...
			printf_s(
				...
				"-x \t\t let you skip the orthogroups that have missing taxonomic headers, which can be usefull for filtering, if you are only interested in files that contain certain species/taxonomic groups.%s"
				"-c \t\t Combines all HOGS into a single OG file: NOT REQUIRED %s"
				"%s"
	...
	{
		printf_s("The Path to the Taxonomic Group File is not set, this means that the program will now generate a fasta file for each Orthogroup without doing a taxonomic group filtering. %s", EOL);
		lRet = glb.propertyBag.SetBoolSkipTaxGroupFiltering();
		if (lRet != ERR_NOERROR)
			return PARSECOMMANDLINE_BOOLTAXGROUPFILTER_OSG;
		aiRequirements[3] = 1;
	...
	{
		printf_s("No skip orthogroups with missing taxonomic headers are set, the program will instead stop the program and tell the user whitch taxonomic header is missing in the taxonomic group file");
		aiRequirements[7] = 1;
	}

	if (aiRequirements[8] == 0)
	{
		printf_s("No concatenation of Orthogroups is set, meaning one fasta file per Hierarchical OrthoGroup (HOG) is created and not per Orthogroup (OG) %s", EOL);
		aiRequirements[8] = 1;
	}
